...
public class Day15CookieRecipes {
	...
	private static void combine(List<Ingredient> ingred, int index, int used, 
            int cap, int dur, int flav, int tex) {
        ...
        if (index == ingred.size() - 1) {
            ...
            //System.out.println("cap:" + cap + " dur:" + dur + " flav:" + flav + " tex:" + tex);
	...
	private static void combineCal(List<Ingredient> ingred, int index, int used, 
            int cap, int dur, int flav, int tex, int cal) {
        Ingredient in = ingred.get(index);
        if (index == ingred.size() - 1) {
            int amt = 100 - used;
            cap += amt * in.capacity;
            ...
            tex += amt * in.texture;
            cal += amt * in.calories;
            if (cal == 500) SCORE = Math.max(SCORE, score(cap, dur, flav, tex));
            return;
        }
        for (int i = 0; i <= 100 - used; i++) {
            int c = i * in.capacity + cap;
	...
	private static int score(int cap, int dur, int flav, int tex) {
		if (cap < 0 || dur < 0 || flav < 0 || tex < 0) return 0;
