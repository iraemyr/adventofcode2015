...
public class Day22RPGWiz {
    ...
    private static void simCombat(boolean player, int bHp, int hHp, int mana, int used, int shield, int poison,
            int recharge, boolean hard) {
        if (used > MIN)
            return;
        if (player && hard) {
            hHp--;
        ...
        if (player) {
            ...
            for (String s : spells) {
                ...
                case "poison":
                    ...
                    break;
                case "recharge":
                    if (recharge > 0 || mana < RECHARGE_COST)
                        continue;
                    actionTaken = true;
